...
public class ControllerExceptionHandler {
    ...
    public ResponseEntity<ApiError> handlerBusinessException(AuthorizationException e){
        ApiError error =  ApiError.builder().code(HttpStatus.UNAUTHORIZED.value()).msg(e.getMessage()).date(new Date()).build();
        return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(error);
    }

    @ExceptionHandler
    public ResponseEntity<ApiError> handlerBusinessException(BusinessException e){
        ApiError error =  ApiError.builder().code(HttpStatus.BAD_REQUEST.value()).msg(e.getMessage()).date(new Date()).build();
